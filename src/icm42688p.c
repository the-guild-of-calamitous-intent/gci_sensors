...
// ICM-42688 configuration values
...
int icm42688p_init(icm42688p_io_t *hw) {
  ...
  // INT1/2: push-pull, Act High
  uint8_t INT_SETUP = 0x10 | 0x08 | 0x02 | 0x01;
  uint8_t config[][2] = {
    ...
    {INT_CONFIG0_REG, 0x20}, // UI_DRDY_INT_CLEAR[5:4] 10: Clear on Sensor Reg Read
    {INT_SOURCE0_REG, 0x08}, // UI_DRDY_INT1_EN
    // GYRO Filter ----------------------------------------
    {GYRO_CONFIG_STATIC2_REG, 0x01}, // disable notch filter
...
int icm42688p_read(icm42688p_io_t *hw, imuf_t *imu) {
  ...
  // Temperature (2 bytes)
  // int16_t temp = ((int16_t)data[0] << 8) | (int16_t)data[1];
