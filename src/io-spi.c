...
int32_t gcis_spi_bus_init(
  ...
  if (port == 0) {
    spi = spi0;
    if (((1 << sdi) & spi0_valid.sdi) == 0) return SPI_INVALID_SDI_PIN;
    if (((1 << sdo) & spi0_valid.sdo) == 0) return SPI_INVALID_SDO_PIN;
    if (((1 << sck) & spi0_valid.sck) == 0) return SPI_INVALID_SCK_PIN;
  }
  else if (port == 1) {
    spi = spi1;
    if (((1 << sdi) & spi1_valid.sdi) == 0) return SPI_INVALID_SDI_PIN;
    if (((1 << sdo) & spi1_valid.sdo) == 0) return SPI_INVALID_SDO_PIN;
    if (((1 << sck) & spi1_valid.sck) == 0) return SPI_INVALID_SCK_PIN;
  }
